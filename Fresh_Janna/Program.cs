    ...
    class Program
    {
        ...
        private static void OnGameUpdate(EventArgs args)
        {
            try
            {
                ...
                {
                    var AutoR = ObjectManager.Get<Obj_AI_Hero>().OrderByDescending(x => x.Health).FirstOrDefault(x => x.HealthPercent < MainMenu._MainMenu.Item("AutoRHP").GetValue<Slider>().Value && x.Distance(Player.ServerPosition) < _R.Range && !x.IsDead);
